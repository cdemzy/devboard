...
export function ProjectEditor({
	...
	useEffect(() => {
		let active = true
		void api<ProjectTag[]>('/project-tags')
			.then((savedTags) => {
	...
	function addTag(value = tagInput) {
		...
		if (!tag) return
		if (tag.length > 40) {
			setError('Tags can be up to 40 characters.')
			return
		}
		...
		if (tags.length >= 20) {
			setError('A project can have up to 20 tags.')
			return
		}
	...
	return (
		...
		>
			<DialogContent className="project-editor-dialog h-[80vh] w-[calc(100%-2rem)] max-w-none p-5 sm:w-[80vw] sm:p-10 md:p-14">
				...
				<form
					className="project-editor-form mx-auto flex h-full max-w-3xl flex-col"
					onSubmit={async (event) => {
						...
						setError('')
						try {
							await save({ name, description: String(form.get('description')), tags })
							close()
						} catch (error) {
				...
				>
					<input
						...
						autoFocus
						className="project-editor-name h-auto border-0 bg-transparent px-0 py-2 text-3xl font-semibold tracking-tight placeholder:text-muted-foreground/55 focus-visible:ring-0 md:text-4xl"
					/>
					<div className="project-editor-description-field mt-8 grid gap-2">
						...
						<textarea
							...
							className="project-editor-description min-h-28 resize-y border-0 bg-transparent px-0 text-base focus-visible:ring-0"
						/>
					</div>
					<div className="project-editor-platform-field mt-7 grid gap-2">
						...
						<div className="project-editor-platform-input relative flex min-h-10 flex-wrap items-center gap-1.5">
							{tags.map((tag) => (
								<span
									key={tag}
									className="project-editor-tag flex items-center gap-1 rounded-full bg-primary/15 px-2.5 py-1 text-xs text-primary"
								>
									<span>{tag}</span>
									<button
										type="button"
										...
										className="project-editor-tag-remove rounded-full hover:text-foreground"
							...
							<input
								...
								aria-label="Add project platform"
								aria-autocomplete="list"
								aria-expanded={suggestionsOpen && matchingSuggestions.length > 0}
								aria-controls="project-tag-suggestions"
								className="project-editor-tag-input h-8 w-32 border-0 bg-transparent px-0 text-sm focus-visible:ring-0"
							/>
							<Button
								type="button"
								variant="ghost"
							...
							{suggestionsOpen && matchingSuggestions.length > 0 && (
								<div
									id="project-tag-suggestions"
									role="listbox"
									className="project-editor-tag-suggestions absolute left-0 top-full z-10 mt-2 w-64 max-h-40 overflow-y-auto rounded-md border border-border bg-[#161b22] p-1 shadow-lg"
								>
									{matchingSuggestions.map((tag) => (
										<button
											...
											className="project-editor-tag-suggestion flex w-full rounded px-2 py-1.5 text-left text-sm hover:bg-accent"
										>
					...
					{error && (
						<p role="alert" className="project-editor-error text-rose-300">
							{error}
						</p>
					)}
					<div className="project-editor-actions mt-auto flex justify-end border-t border-border pt-5">
						{project ? (
							...
							<Tooltip label="Create project">
								<Button size="icon" aria-label="Create project" disabled={busy}>
									{busy ? (
...
export function TaskEditor({
	...
	return (
		...
		>
			<DialogContent
				hideClose
				className="task-editor-dialog h-[80vh] w-[calc(100%-2rem)] max-w-none sm:w-[80vw]"
			>
				<DialogTitle className="text-lg font-semibold">
					{currentTask ? 'Task details' : 'New task'}
				</DialogTitle>
					...
					{currentTask && remove && (
						<Tooltip label="Delete">
							<Button
								...
								variant="ghost"
								size="icon"
								className="task-editor-delete text-rose-300"
								onClick={() => remove(currentTask)}
								disabled={busy}
				...
				<div className="task-editor-fields space-y-4">
					<label>
						...
						<input
							...
							value={draft.title}
							onChange={(event) =>
								updateDraft({ title: capitalizeFirst(event.target.value) })
							}
							onBlur={() => {
								if (dirty) void saveLatest()
							}}
					...
					<div className="task-editor-selects grid grid-cols-1 gap-4 sm:grid-cols-2">
						...
						<label>
							...
							<select
								...
								onChange={(event) =>
									updateDraft({
										complexity: event.target.value as TaskInput['complexity'],
									})
								}
								onBlur={() => {
									if (dirty) void saveLatest()
					...
					{currentTask && (
						<p className="task-editor-metadata text-xs text-muted-foreground">
							Created {new Date(currentTask.created_at).toLocaleDateString()} · Updated{' '}
							{new Date(currentTask.updated_at).toLocaleDateString()}
						</p>
					)}
					{error && (
						<p role="alert" className="task-editor-error text-rose-300">
							{error}
						</p>
					)}
					<p
						className="task-editor-save-status flex items-center gap-2 pt-2 text-xs text-muted-foreground"
						aria-live="polite"
					>
