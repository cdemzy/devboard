...
function ProjectGallery({ projects, open, create }: { projects: Project[]; open: (project: Project) => void; create: () => void }) {
  return <div className="project-gallery p-6 md:p-8"><header className="project-gallery-header mb-7 flex items-center justify-between gap-4"><h1 className="project-gallery-title text-2xl font-semibold tracking-tight">Projects</h1><Button className="project-gallery-create" onClick={create}><Plus size={15} />New project</Button></header>{projects.length === 0 ? <div className="project-gallery-empty flex min-h-64 flex-col items-center justify-center rounded-lg border border-dashed border-border p-8 text-center"><FolderKanban size={30} className="mb-4 text-primary" /><h2 className="text-lg font-semibold">No projects yet</h2><p className="mt-1 text-sm text-muted-foreground">Create a project to start organizing your work.</p></div> : <div className="project-gallery-grid grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">{projects.map((project) => <button key={project.id} onClick={() => open(project)} className="project-card min-h-36 rounded-lg border border-border bg-[#161b22] p-5 text-left transition-colors hover:border-[#484f58] hover:bg-accent"><h2 className="project-card-title text-base font-semibold">{project.name}</h2>{project.tags.length > 0 && <div className="project-card-tags mt-4 flex flex-wrap gap-1.5">{project.tags.map((tag) => <span key={tag} className="project-card-tag rounded-full bg-primary/15 px-2 py-1 text-xs text-primary">{tag}</span>)}</div>}</button>)}</div>}</div>;
}
  ...
  return (
    <div className="workspace-shell flex min-h-screen flex-col md:flex-row">
      <main className="workspace-main flex min-h-screen min-w-0 flex-1 flex-col">
        <header className="workspace-header flex min-h-16 flex-wrap items-center gap-2 border-b border-border bg-[#161b22] px-3 py-2 sm:flex-nowrap sm:gap-4 sm:px-5"><div className="workspace-brand flex shrink-0 items-center gap-2.5 text-base font-semibold tracking-tight"><Layers3 size={22} className="text-primary" />DevBoard</div>{viewHistory.length > 0 && <Tooltip label="Back"><Button className="workspace-back" variant="ghost" size="icon" aria-label="Back" onClick={goBack}><ArrowLeft size={18} /></Button></Tooltip>}<nav className="workspace-nav order-3 flex w-full items-center justify-center gap-1 border-t border-border pt-2 sm:order-none sm:w-auto sm:border-0 sm:pt-0"><button onClick={() => navigate("projects")} className={`workspace-nav-link rounded-md px-3 py-2 text-sm font-medium ${workspaceView === "projects" ? "bg-accent text-foreground" : "text-muted-foreground hover:bg-accent hover:text-foreground"}`}><FolderKanban size={16} className="mr-2 inline" />Projects</button><button onClick={() => navigate("archived")} className={`workspace-nav-link rounded-md px-3 py-2 text-sm font-medium ${workspaceView === "archived" ? "bg-accent text-foreground" : "text-muted-foreground hover:bg-accent hover:text-foreground"}`}><Archive size={16} className="mr-2 inline" /><span className="sm:hidden">Archived</span><span className="hidden sm:inline">Archive</span></button></nav><div className="workspace-account relative ml-auto"><Tooltip label="Account"><Button className="workspace-account-trigger" variant="ghost" size="icon" aria-label="Account" onClick={() => setAccountOpen((open) => !open)}><CircleUserRound size={20} /></Button></Tooltip><AnimatePresence>{accountOpen && <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.16 }} className="workspace-account-menu absolute right-0 top-11 z-30 w-56 rounded-lg border border-border bg-[#161b22] p-2 shadow-xl"><p className="workspace-account-email truncate px-2 py-2 text-xs text-muted-foreground">{email}</p><Button variant="ghost" size="sm" className="workspace-logout w-full justify-start" onClick={async () => { try { const { error } = await getSupabase().auth.signOut(); if (error) throw error; } catch (error) { setError(error instanceof Error ? error.message : "Unable to log out."); } }}><LogOut size={15} />Log out</Button></motion.div>}</AnimatePresence></div></header>
