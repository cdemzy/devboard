...
function SidebarProjectSkeletons({ collapsed }: { collapsed: boolean }) {
  return <div aria-label="Loading projects" className="space-y-1.5">{Array.from({ length: 4 }, (_, index) => <div key={index} aria-hidden="true" className={`flex h-9 animate-pulse items-center rounded-md ${collapsed ? "justify-center" : "gap-2 px-2.5"}`}><span className="h-4 w-4 shrink-0 rounded bg-muted-foreground/20" />{!collapsed && <span className="h-3 flex-1 rounded bg-muted-foreground/20" />}</div>)}</div>;
}
...
export function Workspace({ email }: { email: string }) {
  ...
  return (
    <div className="workspace-shell min-h-screen">
      <aside className={`workspace-sidebar fixed inset-y-0 left-0 z-20 hidden h-dvh flex-col border-r border-border bg-[#161b22] transition-[width] duration-200 md:flex ${isSidebarCollapsed ? "w-16" : "w-64"}`}>
        ...
        <div className={`workspace-sidebar-content flex min-h-0 flex-1 flex-col ${isSidebarCollapsed ? "p-2" : "p-3"}`}>
          {isSidebarCollapsed && <div className="workspace-sidebar-expand-row mb-3 flex justify-center"><Tooltip label="Expand sidebar" side="right"><Button className="workspace-sidebar-expand" variant="ghost" size="icon" aria-label="Expand sidebar" onClick={() => setIsSidebarCollapsed(false)}><PanelLeftOpen size={17} /></Button></Tooltip></div>}
          <div className={`workspace-projects-header mb-2 flex items-center ${isSidebarCollapsed ? "justify-center" : "justify-between px-2"}`}>
            ...
            <Tooltip label="New project" side={isSidebarCollapsed ? "right" : "bottom"}><Button className="workspace-sidebar-create" variant="ghost" size="icon" aria-label="New project" disabled={isCreatingProject} onClick={() => void createEmptyProject()}><Plus size={17} /></Button></Tooltip>
          </div>
          <nav aria-label="Projects" className="workspace-project-list min-h-0 flex-1 space-y-1 overflow-y-auto pr-1">
            {loading ? <SidebarProjectSkeletons collapsed={isSidebarCollapsed} /> : projects.map((item) => { const projectLink = <button key={item.id} onClick={() => selectProject(item.id)} aria-label={isSidebarCollapsed ? item.name : undefined} aria-current={workspaceView === "board" && active === item.id ? "page" : undefined} className={`workspace-project-link flex w-full items-center rounded-md py-2 text-left text-sm transition-colors ${isSidebarCollapsed ? "justify-center px-0" : "gap-2 px-2.5"} ${workspaceView === "board" && active === item.id ? "bg-accent text-foreground" : "text-muted-foreground hover:bg-accent hover:text-foreground"}`}><FolderKanban size={15} className="shrink-0" />{!isSidebarCollapsed && <span className="truncate">{item.name}</span>}</button>; return isSidebarCollapsed ? <Tooltip key={item.id} label={item.name} side="right" className="w-full">{projectLink}</Tooltip> : projectLink; })}
          </nav>
          <div className="workspace-sidebar-footer mt-auto border-t border-border pt-3">
            <Tooltip label="Archived projects" side={isSidebarCollapsed ? "right" : "bottom"} className="w-full"><button onClick={openArchive} className={`workspace-archive-link flex w-full items-center rounded-md py-2 text-left text-sm transition-colors ${isSidebarCollapsed ? "justify-center px-0" : "gap-2 px-2.5"} ${workspaceView === "archived" ? "bg-accent text-foreground" : "text-muted-foreground hover:bg-accent hover:text-foreground"}`}><Archive size={15} />{!isSidebarCollapsed && "Archived projects"}</button></Tooltip>
            <div className="workspace-account mt-2"><AccountMenu email={email} open={accountOpen} close={() => setAccountOpen(false)} reportError={setError} variant="sidebar" /><Tooltip label="Account" side={isSidebarCollapsed ? "right" : "bottom"} className="w-full"><button type="button" className={`workspace-account-trigger flex w-full items-center rounded-md py-2 text-left text-sm text-muted-foreground transition-colors hover:bg-accent hover:text-foreground ${isSidebarCollapsed ? "justify-center px-0" : "gap-2 px-2.5"}`} aria-label="Account" onClick={() => { if (isSidebarCollapsed) setIsSidebarCollapsed(false); setAccountOpen((open) => !open); }}><CircleUserRound size={17} />{!isSidebarCollapsed && <span className="truncate">Account</span>}</button></Tooltip></div>
          </div>
        </div>
      </aside>
      <main className={`workspace-main flex min-h-screen min-w-0 flex-1 flex-col transition-[margin-left,width] duration-200 ease-in-out md:h-dvh md:min-h-0 md:overflow-x-hidden md:overflow-y-auto ${isSidebarCollapsed ? "md:ml-16 md:w-[calc(100%-4rem)]" : "md:ml-64 md:w-[calc(100%-16rem)]"}`}>
