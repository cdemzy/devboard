...
export function Workspace({ email }: { email: string }) {
  ...
  return (
    <div className="workspace-shell flex min-h-screen flex-col md:flex-row">
      <aside className={`workspace-sidebar hidden h-dvh shrink-0 flex-col border-r border-border bg-[#161b22] transition-[width] duration-200 md:flex ${isSidebarCollapsed ? "w-16" : "w-64"}`}>
        ...
        <div className={`workspace-sidebar-content flex min-h-0 flex-1 flex-col ${isSidebarCollapsed ? "p-2" : "p-3"}`}>
          ...
          <div className={`workspace-projects-header mb-2 flex items-center ${isSidebarCollapsed ? "justify-center" : "justify-between px-2"}`}>
            ...
            <Tooltip label="New project"><Button className="workspace-sidebar-create" variant="ghost" size="icon" aria-label="New project" onClick={() => setCreating(true)}><Plus size={17} /></Button></Tooltip>
          </div>
      ...
      {creating && <ProjectEditor close={() => setCreating(false)} save={async (data) => { const created = await api<Project>("/projects", json("POST", data)); setProjects((previous) => [...previous, created]); setActive(created.id); setWorkspaceView("board"); }} />}
