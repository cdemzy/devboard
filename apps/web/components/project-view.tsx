...
const tagColorValues = { default: "#37352F", gray: "#9B9A97", brown: "#64473A", orange: "#D9730D", yellow: "#DFAB01", green: "#0F7B6C", blue: "#0B6E99", purple: "#6940A5", pink: "#AD1A72", red: "#E03E3E" } as const;
...
export function ProjectView({
  ...
  useEffect(() => {
    void api<ProjectTag[]>("/project-tags").then(setTagSuggestions).catch(() => undefined);
  }, []);
          ...
          <div className="min-w-0 max-w-3xl flex-1">
            ...
            <div ref={tagMenuRef} className="relative mt-3">
              <div role="button" tabIndex={0} onMouseDown={(event) => { if (event.target === event.currentTarget) event.preventDefault(); }} onClick={() => setTagsOpen(true)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setTagsOpen(true); } }} className={`flex min-h-9 cursor-pointer flex-wrap items-center gap-1.5 !outline-none [-webkit-tap-highlight-color:transparent] focus:!outline-none ${tagsOpen ? "rounded-t-md border border-border bg-accent px-2 py-2" : "rounded-md px-1 py-1"}`} aria-label="Edit project tags" aria-expanded={tagsOpen}>
                {projectDraft.tags.length === 0 && !tagsOpen ? <span className="px-1 text-xs text-muted-foreground">Add tags</span> : projectDraft.tags.map((tag) => { const catalog = tagSuggestions.find((item) => item.name.toLowerCase() === tag.toLowerCase()); return <span key={tag} style={{ backgroundColor: tagColorValues[catalog?.color ?? "default"] }} className="flex items-center gap-1 rounded-sm px-2 py-1 text-xs text-white">{tag}<button type="button" onClick={(event) => { event.stopPropagation(); setProjectDraft((current) => ({ ...current, tags: current.tags.filter((item) => item !== tag) })); }} aria-label={`Remove ${tag} tag`} className="rounded-sm text-white/65 hover:text-white"><X size={12} /></button></span>; })}
              ...
              {tagsOpen && <div role="dialog" aria-label="Project tag options" className="absolute inset-x-0 top-full z-20 rounded-b-md border border-t-0 border-border bg-[#161b22] p-2 shadow-xl">
                ...
                {matchingTags.length > 0 && <div className="flex flex-wrap gap-1.5">{matchingTags.map((tag) => { const selected = projectDraft.tags.some((item) => item.toLowerCase() === tag.name.toLowerCase()); return <div key={tag.id} style={{ backgroundColor: tagColorValues[tag.color] }} className="relative flex items-center rounded-sm text-white"><button type="button" onClick={() => { setTagMenuId(null); toggleTag(tag.name); }} className="px-2.5 py-1 text-xs">{tag.name}</button><button type="button" onClick={(event) => { event.stopPropagation(); setTagMenuId(tagMenuId === tag.id ? null : tag.id); }} aria-label={`Tag options for ${tag.name}`} className="mr-1 rounded-sm p-0.5 text-white/70 hover:text-white"><Ellipsis size={14} /></button>{tagMenuId === tag.id && <div className="absolute left-0 top-full z-30 mt-1 w-36 rounded-md border border-border bg-[#161b22] p-1.5 text-foreground shadow-xl"><div className="mb-1.5 grid grid-cols-5 gap-1">{Object.entries(tagColorValues).map(([color, value]) => <button key={color} type="button" aria-label={`Set ${tag.name} to ${color}`} onClick={() => void updateTagColor(tag, color as ProjectTag["color"])} style={{ backgroundColor: value }} className="h-5 rounded-sm" />)}</div><button type="button" onClick={() => void deleteTag(tag)} className="flex w-full items-center gap-1.5 rounded px-1.5 py-1 text-left text-xs text-rose-300 hover:bg-accent"><Trash2 size={13} />Delete?</button></div>}</div>; })}</div>}
