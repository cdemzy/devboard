...
function SortableProjectTag({
  ...
  return (
    <div ref={setNodeRef} style={style} className={`project-tag-option border-0 ${isDragging ? "opacity-60" : ""}`}>
      <div style={{ backgroundColor: tagColorValues[tag.color], fontSize: "12px", lineHeight: 1 }} className="project-tag-chip relative flex border-0 items-center rounded-sm text-white">
      ...
      <div className="project-view mx-auto w-4/5 px-0 pt-5 pb-5 sm:w-full sm:px-5 sm:pt-8 sm:pb-6 md:px-8">
        <header className="project-header mb-7 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between sm:gap-5">
          <div className="min-w-0 w-full max-w-3xl flex-1">
            ...
            <div className="project-platform-section mt-3 flex items-center gap-3">
              ...
              {tagsOpen && <div role="dialog" aria-label="Project tag options" className="project-tag-options absolute inset-x-0 top-full z-20 rounded-b-md border border-t-0 border-border bg-[#161b22] p-2 shadow-xl">
                <input autoFocus value={tagInput} onChange={(event) => setTagInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); const exactMatch = tagSuggestions.find((tag) => tag.name.toLowerCase() === tagInput.trim().toLowerCase()); if (exactMatch) toggleTag(exactMatch.name); else addTag(); } }} aria-label="Search or create a project tag" maxLength={40} placeholder="Search for a tag…" className="project-tag-options-search mb-2 h-8 w-full !border !border-border !bg-background px-2 text-xs !outline-none focus:!outline-none" />
