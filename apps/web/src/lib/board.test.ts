...
describe("optimistic board movement", () => {
  ...
  it("reorders in both directions and keeps contiguous positions", () => {
    ...
    expect(columnTasks(moved, "todo").map((t) => t.id)).toEqual([
      "b",
      "c",
      "a",
    ]);
    expect(
      columnTasks(moveTask(moved, "a", "todo", 0), "todo").map((t) => t.id),
    ).toEqual(["a", "b", "c"]);
    expect(moved.map((t) => t.position)).toEqual([0, 1, 2]);
  });
  it("clamps a stale target position", () => {
    expect(moveTask([task("a", "todo", 0)], "a", "done", 20)[0].position).toBe(
      0,
    );
  });
});
