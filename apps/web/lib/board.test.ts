...
import { getStatusTasks, moveTask } from './board'
...
describe('optimistic board movement', () => {
	it('moves into an empty column without mutating the rollback snapshot', () => {
		...
		expect(getStatusTasks(next, 'done').map((t) => t.id)).toEqual(['a'])
		expect(getStatusTasks(next, 'todo')[0].position).toBe(0)
	...
	it('reorders in both directions and keeps contiguous positions', () => {
		...
		expect(getStatusTasks(moved, 'todo').map((t) => t.id)).toEqual(['b', 'c', 'a'])
		expect(
			getStatusTasks(moveTask(moved, 'a', 'todo', 0), 'todo').map((t) => t.id),
		).toEqual(['a', 'b', 'c'])
